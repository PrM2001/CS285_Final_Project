...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    """
    1. create an initial state x0, H, and the reset time M as configurations 
    2. sample an action from our policy
    3. using this action, we can call the DDP agent using the action as the perturbation
    4. Evaluate this DDP agent K times 
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if next_observation.ndim == 3:
            tmp_observation = next_observation[-1]
        else:
            tmp_observation = next_observation
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(action=action, reward=reward, next_observation=tmp_observation, done=done and not truncated)
        else:
            # We're using the regular replay buffer
            replay_buffer.insert(observation=observation, action=action, next_observation=tmp_observation, done=done and not truncated, reward=reward)
